...
import { getBlogPosts, getProjects, getServices, getSiteSettings, pickLocale } from '@/lib/content';
    ...
    <div className="space-y-16 pb-16">
      ...
      <section className="container space-y-8" dir={dir}>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {projects.map((project) => (
            <Link key={project.id} href={`/${locale}/projects/${project.id}`} className="card-surface overflow-hidden group">
              <div className="relative h-64">
                <Image src={project.mainImageUrl} alt={project.titleEn} fill className="object-cover transition-transform duration-500 group-hover:scale-105" />
              </div>
              <div className="p-6 space-y-2" dir={dir}>
                <div className="text-sm text-brand-orange font-semibold">{project.type}</div>
                <h3 className="text-2xl font-semibold">{pickLocale(project, locale, { ar: 'titleAr', en: 'titleEn' })}</h3>
                <p className="text-gray-600 line-clamp-2">{pickLocale(project, locale, { ar: 'descriptionAr', en: 'descriptionEn' })}</p>
                <div className="text-sm text-gray-500">{pickLocale(project, locale, { ar: 'locationAr', en: 'locationEn' })}</div>
              </div>
            </Link>
          ))}
        </div>
      ...
      <section className="container space-y-8" dir={dir}>
        <SectionHeading
          ...
          subtitle={locale === 'ar' ? 'أفكار حول الفخامة الهادئة والابتكار المعماري.' : 'Insights on quiet luxury and architectural innovation.'}
        />
        <div className="grid md:grid-cols-3 gap-6">
          {posts.map((post) => (
